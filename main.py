...
app = FastAPI(title="AI Lead Monster", version="6.0.0", lifespan=lifespan)
        ...
        return {"leads": [
            {
                ...
                "content": l.content[:500], "url": l.source_url,
...
async def scan_jobs(x_admin_key: str = Header(None)):
    verify_admin(x_admin_key)
    try:
        from scanners.job_scanner import JobBoardScanner
        scanner = JobBoardScanner()
        result = await scanner.scan()
        return result
    except Exception as e:
        logger.error(f"Jobs scan error: {e}")
        raise HTTPException(500, detail=str(e))

@app.post("/api/scan/all")
async def scan_all(x_admin_key: str = Header(None)):
    verify_admin(x_admin_key)
    try:
        from scanners.telegram_scanner import TelegramScanner
        from scanners.job_scanner import JobBoardScanner
        
        tg_scanner = TelegramScanner()
        job_scanner = JobBoardScanner()
        
        tg_res = await tg_scanner.scan()
        job_res = await job_scanner.scan()
        
        return {
            "telegram": tg_res,
            "jobs": job_res,
            "total_found": tg_res["found"] + job_res["found"],
            "total_hot": tg_res["hot"] + job_res["hot"]
        }
    except Exception as e:
        logger.error(f"All-scan error: {e}")
        raise HTTPException(500, detail=str(e))
